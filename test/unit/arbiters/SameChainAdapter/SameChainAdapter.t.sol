// SPDX-License-Identifier: MIT
pragma solidity ^0.8.28;

// Base
import { Test } from "forge-std/Test.sol";

// Contracts
import { SameChainAdapter } from "@rhinestone/compact-utils/src/arbiters/samechain/SameChainAdapter.sol";
import { SameChainArbiter } from "@rhinestone/compact-utils/src/arbiters/samechain/SameChainArbiter.sol";
import { AddressBook } from "@rhinestone/compact-utils/src/common/AddressBook/AddressBook.sol";
import { IntentExecutor } from "src/executor/IntentExecutor.sol";
import { Types } from "@rhinestone/compact-utils/src/types/OrderTypes.sol";
import { SmartExecutionLib } from "@rhinestone/compact-utils/src/common/SmartExecutionLib.sol";
import { Constants } from "@rhinestone/compact-utils/src/types/Constants.sol";
import { AddressBookLib } from "@rhinestone/compact-utils/src/common/AddressBook/lib/AddressBookLib.sol";
import { Execution } from "modulekit/integrations/ERC7579Exec.sol";
import { TheCompact } from "the-compact/TheCompact.sol";
import { AlwaysOKAllocator } from "the-compact/test/AlwaysOKAllocator.sol";
import { Paymaster } from "@rhinestone/compact-utils/src/executor/StandaloneIntent/aux/Paymaster.sol";

// Interfaces
import { IERC20 } from "forge-std/interfaces/IERC20.sol";
import { IPermit2 } from "permit2/src/interfaces/IPermit2.sol";
import { ITheCompact } from "the-compact/interfaces/ITheCompact.sol";

// Mocks
import { MockERC20 } from "@rhinestone/compact-utils/src/tests/MockERC20.sol";
import { MockTarget } from "@rhinestone/compact-utils/src/tests/MockTarget.sol";
import { Mock1271Account } from "test/utils/mocks/Mock1271Account.sol";

// Libraries
import { IdLib } from "the-compact/lib/IdLib.sol";
import { EfficiencyLib } from "the-compact/lib/EfficiencyLib.sol";

contract SameChainAdapter_Unit_Test is Test {
    /* //////////////////////////////////////////////////////////////
                               LIBRARIES
    //////////////////////////////////////////////////////////////*/

    using IdLib for *;
    using EfficiencyLib for *;

    /* //////////////////////////////////////////////////////////////
                                  STATE
    //////////////////////////////////////////////////////////////*/

    SameChainAdapter internal sameChainAdapter;
    SameChainArbiter internal sameChainArbiter;
    AddressBook internal addressBook;
    IntentExecutor internal intentExecutor;
    MockTarget internal target;
    IPermit2 internal permit2;

    address internal router;
    address internal compact;
    address internal allocator;
    address internal sameChainArbiterAddress;

    MockERC20 internal tokenA;
    MockERC20 internal tokenB;
    MockERC20 internal tokenC;

    address internal solver;
    address internal sponsor;
    address internal recipient;
    address internal tokenInRecipient;
    uint96 allocatorId;
    bytes12 internal lockTag;

    uint256 internal constant CHAIN_ID = 1;

    /* //////////////////////////////////////////////////////////////
                                SETUP
    //////////////////////////////////////////////////////////////*/

    function setUp() public virtual {
        // Set up addresses
        router = makeAddr("router");
        solver = makeAddr("solver");
        sponsor = address(new Mock1271Account(true));
        vm.label(sponsor, "sponsor");
        recipient = makeAddr("recipient");
        tokenInRecipient = makeAddr("tokenInRecipient");
        allocator = address(new AlwaysOKAllocator());
        sameChainArbiterAddress = makeAddr("sameChainArbiter");

        // Deploy Permit2 first
        _deployPermit2();
        permit2 = IPermit2(address(Constants.PERMIT2));

        // Deploy AddressBook
        addressBook = new AddressBook(address(this));

        // Deploy Compact
        compact = address(new TheCompact());
        allocatorId = ITheCompact(compact).__registerAllocator(allocator, "");
        // Create lock tag (scope=0, resetPeriod=1)
        lockTag = bytes12(
            (uint96(0) << 88) // scope (Multichain)
                | (uint96(1) << 80) // reset period (SevenDaysAndOneHour)
                | allocatorId // allocator ID
        );

        // Deploy target
        target = new MockTarget();

        // CRITICAL: Set the SameChainArbiter address in AddressBook BEFORE deploying IntentExecutor
        // This is needed because TrustedExecution constructor reads it
        addressBook.setAddress(Constants.SAMECHAIN_ARBITER_ID, sameChainArbiterAddress);

        // Deploy Paymaster and register in AddressBook for StandaloneIntentExecutor
        address predictedIntentExecutor = address(0x1234); // Placeholder - will be replaced
        Paymaster paymaster = new Paymaster(predictedIntentExecutor, address(this));
        addressBook.setAddress(Constants.PAYMASTER_ID, address(paymaster));

        // Deploy real IntentExecutor (will read sameChainArbiterAddress from AddressBook)
        intentExecutor = new IntentExecutor(router, compact, allocator, address(addressBook), address(0xdeadbeef));

        // Set the executor in the AddressBook
        addressBook.setAddress(Constants.INTENT_EXECUTOR_ID, address(intentExecutor));

        // NOW deploy the real SameChainAdapter (will read executor from AddressBook)
        sameChainAdapter = new SameChainAdapter(router, compact, sameChainArbiterAddress, address(addressBook));

        // Etch the real arbiter code to the placeholder address
        vm.etch(sameChainArbiterAddress, address(sameChainAdapter).code);
        sameChainAdapter = SameChainAdapter(sameChainArbiterAddress);

        // Deploy mock tokens
        tokenA = new MockERC20("TokenA", "TKA", 18);
        tokenB = new MockERC20("TokenB", "TKB", 18);
        tokenC = new MockERC20("TokenC", "TKC", 18);
        vm.label(address(tokenA), "TokenA");
        vm.label(address(tokenB), "TokenB");
        vm.label(address(tokenC), "TokenC");

        // Setup initial balances
        tokenA.mint(solver, 1000 ether);
        tokenB.mint(solver, 1000 ether);
        tokenC.mint(solver, 1000 ether);
        tokenA.mint(sponsor, 1000 ether);
        tokenB.mint(sponsor, 1000 ether);
        tokenC.mint(sponsor, 1000 ether);

        // Fund with ETH
        vm.deal(solver, 10 ether);
        vm.deal(router, 10 ether);
        vm.deal(sponsor, 10 ether);

        // Approve Permit2 from sponsor
        vm.startPrank(sponsor);
        tokenA.approve(address(permit2), type(uint256).max);
        tokenB.approve(address(permit2), type(uint256).max);
        tokenC.approve(address(permit2), type(uint256).max);
        vm.stopPrank();
    }

    /* //////////////////////////////////////////////////////////////
                            PERMIT2 DEPLOYMENT
    //////////////////////////////////////////////////////////////*/

    function _deployPermit2() internal {
        address permit2Deployer = address(0x4e59b44847b379578588920cA78FbF26c0B4956C);
        address deployedPermit2Deployer;
        address permit2DeployerDeployer = address(0x3fAB184622Dc19b6109349B94811493BF2a45362);
        bytes memory permit2DeployerCreationCode =
            hex"604580600e600039806000f350fe7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffe03601600081602082378035828234f58015156039578182fd5b8082525050506014600cf3";
        vm.deal(permit2DeployerDeployer, 1e18);
        vm.prank(permit2DeployerDeployer);
        assembly ("memory-safe") {
            deployedPermit2Deployer := create(0, add(permit2DeployerCreationCode, 0x20), mload(permit2DeployerCreationCode))
        }

        require(deployedPermit2Deployer != permit2Deployer, "Contract deployment failed");

        if (true) {
            bytes memory permit2CreationCalldata =
                hex"";

            (bool ok,) = permit2Deployer.call(permit2CreationCalldata);
            require(ok && address(Constants.PERMIT2).code.length != 0, "permit2 deployment failed");
        }
        vm.label(address(Constants.PERMIT2), "PERMIT2");
    }

    /* //////////////////////////////////////////////////////////////
                                HELPERS
    //////////////////////////////////////////////////////////////*/

    function _prankDelegateCall() internal {
        // Etch adapter code to the router address for delegate call simulation
        vm.etch(router, address(sameChainAdapter).code);
        sameChainAdapter = SameChainAdapter(payable(router));
    }

    function _createBasicOrder() internal view returns (Types.Order memory) {
        Types.Operation memory emptyOp = Types.Operation({ data: "" });

        uint256[2][] memory tokenIn = new uint256[2][](1);
        tokenIn[0] = [_toId(address(tokenA)), 100 ether];

        uint256[2][] memory tokenOut = new uint256[2][](1);
        tokenOut[0] = [_toId(address(tokenB)), 50 ether];

        return Types.Order({
            sponsor: sponsor,
            recipient: recipient,
            nonce: 1,
            expires: block.timestamp + 1 hours,
            fillDeadline: block.timestamp + 1 hours,
            notarizedChainId: CHAIN_ID,
            targetChainId: CHAIN_ID,
            tokenIn: tokenIn,
            tokenOut: tokenOut,
            packedGasValues: 0,
            preClaimOps: emptyOp,
            targetOps: emptyOp,
            qualifier: ""
        });
    }

    function _toId(address token) internal view returns (uint256 id) {
        lockTag.toAllocatorId();
        id = lockTag.asUint256() | token.asUint256();
    }

    function _createOrderWithMultipleTokensIn() internal view returns (Types.Order memory) {
        Types.Operation memory emptyOp = Types.Operation({ data: "" });

        uint256[2][] memory tokenIn = new uint256[2][](2);
        tokenIn[0] = [_toId(address(tokenA)), 100 ether];
        tokenIn[1] = [_toId(address(tokenB)), 50 ether];

        uint256[2][] memory tokenOut = new uint256[2][](1);
        tokenOut[0] = [_toId(address(tokenC)), 75 ether];

        return Types.Order({
            sponsor: sponsor,
            recipient: recipient,
            nonce: 1,
            expires: block.timestamp + 1 hours,
            fillDeadline: block.timestamp + 1 hours,
            notarizedChainId: CHAIN_ID,
            targetChainId: CHAIN_ID,
            tokenIn: tokenIn,
            tokenOut: tokenOut,
            packedGasValues: 0,
            preClaimOps: emptyOp,
            targetOps: emptyOp,
            qualifier: ""
        });
    }

    function _createBasicSignatures() internal pure returns (Types.Signatures memory) {
        return Types.Signatures({ notarizedClaimSig: abi.encodePacked(bytes32(0), bytes32(0), uint8(27)), preClaimSig: "" });
    }

    function _executeHandleFillCompact(SameChainAdapter.FillDataCompact memory fillData) internal returns (bytes4) {
        // Encode solver context (tokenInRecipient)
        bytes memory relayerContext = abi.encodePacked(tokenInRecipient);

        // Encode the function call
        bytes memory adapterCalldata = abi.encodeWithSelector(SameChainAdapter.samechain_compact_handleFill.selector, fillData);

        // Append solver context as router would do
        bytes memory fullCalldata = abi.encodePacked(adapterCalldata, relayerContext, uint256(relayerContext.length));

        _prankDelegateCall();

        // Perform the delegate call as router
        vm.prank(solver);
        (bool success, bytes memory returnData) = router.call(fullCalldata);
        success; // To silence solc warning
        return abi.decode(returnData, (bytes4));
    }

    function _executeHandleFillPermit2(SameChainAdapter.FillDataPermit2 memory fillData) internal returns (bytes4) {
        // Encode solver context (tokenInRecipient)
        bytes memory relayerContext = abi.encodePacked(tokenInRecipient);

        // Encode the function call
        bytes memory adapterCalldata = abi.encodeWithSelector(SameChainAdapter.samechain_permit2_handleFill.selector, fillData);

        // Append solver context as router would do
        bytes memory fullCalldata = abi.encodePacked(adapterCalldata, relayerContext, uint256(relayerContext.length));

        _prankDelegateCall();

        // Perform the delegate call as router
        vm.prank(solver);
        (bool success, bytes memory returnData) = router.call(fullCalldata);
        success; // To silence solc warning
        return abi.decode(returnData, (bytes4));
    }

    function _encodePreClaimOperationsERC7579() internal view returns (bytes memory) {
        Execution[] memory executions = new Execution[](2);
        executions[0] =
            Execution({ target: address(tokenC), value: 0, callData: abi.encodeCall(IERC20.approve, (address(target), 50 ether)) });
        executions[1] = Execution({ target: address(target), value: 0, callData: abi.encodeCall(MockTarget.targetFn, (12_345)) });

        return abi.encodePacked(
            uint8(SmartExecutionLib.Type.ERC7579), uint8(SmartExecutionLib.SigMode.EMISSARY_ERC1271), abi.encode(executions)
        );
    }

    function _encodeMultiplePreClaimOperationsERC7579() internal view returns (bytes memory) {
        Execution[] memory executions = new Execution[](2);
        executions[0] =
            Execution({ target: address(tokenC), value: 0, callData: abi.encodeCall(IERC20.approve, (address(target), 30 ether)) });
        executions[1] = Execution({ target: address(tokenC), value: 0, callData: abi.encodeCall(IERC20.approve, (solver, 20 ether)) });

        return abi.encodePacked(
            uint8(SmartExecutionLib.Type.ERC7579), uint8(SmartExecutionLib.SigMode.EMISSARY_ERC1271), abi.encode(executions)
        );
    }

    function _encodeTargetOperationsERC7579() internal view returns (bytes memory) {
        Execution[] memory executions = new Execution[](1);
        executions[0] =
            Execution({ target: address(tokenB), value: 0, callData: abi.encodeCall(IERC20.transfer, (address(target), 20 ether)) });

        return abi.encodePacked(
            uint8(SmartExecutionLib.Type.ERC7579), uint8(SmartExecutionLib.SigMode.EMISSARY_ERC1271), abi.encode(executions)
        );
    }

    function _encodeMultipleTargetOperationsERC7579() internal view returns (bytes memory) {
        Execution[] memory executions = new Execution[](2);
        executions[0] =
            Execution({ target: address(tokenB), value: 0, callData: abi.encodeCall(IERC20.transfer, (address(target), 10 ether)) });
        executions[1] = Execution({ target: address(tokenB), value: 0, callData: abi.encodeCall(IERC20.transfer, (solver, 5 ether)) });

        return abi.encodePacked(
            uint8(SmartExecutionLib.Type.ERC7579), uint8(SmartExecutionLib.SigMode.EMISSARY_ERC1271), abi.encode(executions)
        );
    }
}
